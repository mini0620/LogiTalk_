...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.btn = CTkButton(
            self, text='▶️', command=self.toggle_show_menu, width=30)
        ...
        self.open_img_button = CTkButton(
            self, text='📂', width=50, height=40, command=self.open_image)
        ...
        try:
            ...
            self.sock.connect(('0.tcp.eu.ngrok.io', 14084))
    ...
    def toggle_show_menu(self):
        if self.is_show_menu:
            ...
            self.entry = CTkEntry(
                self.menu_frame, placeholder_text="Ваш нік...")
            self.entry.pack()
            self.save_button = CTkButton(
                self.menu_frame, text="Зберегти", command=self.save_name)
    ...
    def adaptive_ui(self):
        ...
        self.send_button.place(x=self.winfo_width() - 50,
        ...
        self.open_img_button.place(
            x=self.winfo_width()-105, y=self.send_button.winfo_y())
    ...
    def open_image(self):
        ...
        try:
            ...
            self.sock.sendall(data.encode())
            self.add_message('', CTkImage(
                light_image=Image.open(file_name), size=(300, 300)))
        except Exception as e:
